...
namespace cc::graphics
{
	...
	bool ConstantBuffer::Create(size_t size)
	{
		...
		cc::graphics::GetDevice()->CreateBuffer(buffer.GetAddressOf(), &desc, nullptr);
	...
	{
		cc::graphics::GetDevice()->SetConstantBuffer(buffer.Get(), data, desc.ByteWidth);
	}

	void ConstantBuffer::Bind(eShaderStage stage)
	{
		cc::graphics::GetDevice()->BindConstantBuffer(stage, mType, buffer.Get());
	}

...
namespace cc
{
	...
	public:
		...
		static std::shared_ptr<T> Find(const std::wstring& key)
		{
			...
			//데이터가 없다면 end를 반환해준다.
			std::map<std::wstring, std::shared_ptr<Resource>>::iterator iter = mResourceManager.find(key);
	...
	private:
		static std::map<std::wstring, std::shared_ptr<Resource>> mResourceManager;
	};
}
